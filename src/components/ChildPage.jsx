...
// Frontendda receipt obyektini o'zi generatsiya qilish
function makeReceipt(child) {
  return {
    sessionId: child._id,
    token: child.token_code || child.qr_code,
    printed_at: new Date(),
    entry_time: child.entry_time,
    paid_until: child.paid_until,
    exit_time: new Date(), // checkout payti
    included_minutes: Math.floor(
      (new Date(child.paid_until) - new Date(child.entry_time)) / 60000
    ),
    extra_minutes: 0,
    per_minute: 833, // misol uchun
    currency: "UZS",
    base_amount: 50000, // misol uchun
    extra_fee: 0,
    total: 50000, // misol uchun
    rounding: { strategy: "-", round_to: "-" },
  };
}
...
export default function ChildPage() {
  ...
  const handleCheckoutAndPrint = async () => {
    try {
      ...
      // Backenddan receipt olish o‘rniga frontendda generatsiya qilamiz
      const receipt = makeReceipt(child);
      printReceiptClient(receipt);
  ...
  const handleReprint = async () => {
    try {
      const res = await api.post(`/api/children/${child._id}/reprint`);
  ...
  const handleScan = async (qrToken) => {
    try {
      const res = await childrenApi.scan(qrToken);
      // ...scan natijasini ko‘rsatish...

      // Agar checkout bo‘lsa (masalan, res.status === "checkout" yoki kerakli shart)
